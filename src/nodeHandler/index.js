...
const NodeHandler = {
        ...
        for (const node of nodeIterator.node.body) {
            if (node.type === 'VariableDeclaration' && node.kind === 'var') {
                for (const declaration of node.declarations) {
                    if (declaration.init) {
                        scope.declare(declaration.id.name, nodeIterator.traverse(declaration.init, {
                            scope
                        }), node.kind)
                    } else {
    ...
    FunctionExpression(nodeIterator) {
        ...
        const fn = function () {
            ...
            const signal = nodeIterator.traverse(node.body, {
                scope
            })
    ...
    UpdateExpression(nodeIterator) {
        let {
            value
        } = nodeIterator.scope.get(nodeIterator.node.argument.name)
    ...
    ForStatement(nodeIterator) {
        ...
        for (
            node.init && nodeIterator.traverse(node.init, {
                scope
            }); node.test ? nodeIterator.traverse(node.test, {
                scope
            }) : true; node.update && nodeIterator.traverse(node.update, {
                scope
            })
        ) {
            const signal = nodeIterator.traverse(node.body, {
    ...
    ForInStatement(nodeIterator) {
        ...
        for (const key in nodeIterator.traverse(right)) {
            value.value = key
            const signal = nodeIterator.traverse(body, {
                scope
            })
